...
const auditInsert = sql.match(/insert\s+into\s+public\.audit_log[\s\S]*?\)\s*values\s*\([\s\S]*?\);/i)?.[0] || '';
check('raw token is not written to audit log', !/p_access_token|token_hash|lower\s*\(\s*p_access_token\s*\)/i.test(auditInsert));
