...
Deno.serve(async(req:Request)=>{
  ...
  const {error:insertError}=await db.from("marketing_oauth_states").insert({provider,state_hash:await sha256Hex(state),code_verifier:verifier,redirect_uri:CALLBACK,requested_scopes:[scope],created_by_team_id:member.id,return_to:CRM_RETURN,expires_at:expiresAt});
