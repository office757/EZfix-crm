...
Deno.serve(async(req:Request)=>{
  ...
  if(!clientId||!clientSecret) return J({ok:false,error:"Google Ads OAuth credentials are incomplete",code:"CREDENTIALS_MISSING"},409);
  ...
  const listRes=await fetch(`https://googleads.googleapis.com/${API_VERSION}/customers:listAccessibleCustomers`,{
    headers:{Authorization:`Bearer ${accessToken}`,...(developerToken?{"developer-token":developerToken}:{}),"Content-Type":"application/json"}
  });
