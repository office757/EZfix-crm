...
const sha256Hex=async(v:string)=>Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",new TextEncoder().encode(v)))).map(b=>b.toString(16).padStart(2,"0")).join("");
const clientIpOf=(req:Request)=>clean((req.headers.get("cf-connecting-ip")||req.headers.get("x-real-ip")||req.headers.get("x-forwarded-for")||"").split(",")[0],128);
Deno.serve(async(req)=>{
 ...
 const declared=Number(req.headers.get("content-length")||0);if(Number.isFinite(declared)&&declared>50000)return reply("Request could not be processed.",413);
 ...
 const honeypot=clean(pick(b,["website","company_website","company-website","homepage","fax_number","fax-number"]),300);if(honeypot)return reply(success);
