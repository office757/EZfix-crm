...
begin
  ...
  if transfer_status = 'forwarded' then
    update public.tasks
       set status = 'completed',
           updated_at = now(),
           app_data = coalesce(app_data, '{}'::jsonb) || jsonb_build_object(
             'transfer_status', transfer_status,
             'resolved_by', 'provider_forwarding',
             'resolved_at', now()
           )
     where id = task_key
       and status = 'open';
  ...
  on conflict (id) do update
     set customer_id = coalesce(excluded.customer_id, public.tasks.customer_id),
         priority = 'high',
         updated_at = now(),
         app_data = coalesce(public.tasks.app_data, '{}'::jsonb) || excluded.app_data
...
drop trigger if exists trg_calls_ai_human_followup on public.calls;
create trigger trg_calls_ai_human_followup
after insert or update of lead_extraction, customer_id, lead_id on public.calls
for each row
execute function public.enqueue_ai_receptionist_human_followup();
